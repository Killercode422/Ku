...
def welcome_start(message):
    ...
    # First button row
    keyboard.row(
        telebot.types.InlineKeyboardButton('💸 BUY HACK', url='https://t.me/'),
        telebot.types.InlineKeyboardButton('📷 SEND SCREENSHOT', url='https://t.me/')
    )
    
    # Second button row
    keyboard.row(
        telebot.types.InlineKeyboardButton('🎓 How To Use', url='t.me/SIDIKI_MUSTAFA_47'),
        telebot.types.InlineKeyboardButton('🆔 Telegram ID', url='t.me/SIDIKI_MUSTAFA_47')
    )
...
def validate_and_execute_attack(message, target, port, time):
    ...
    if len(command) > 0:
        ...
        if validate_key(user_key):
            ...
            keyboard = telebot.types.InlineKeyboardMarkup()
            join_button = telebot.types.InlineKeyboardButton('🚩 Join Channel 🚩', url='t.me/SIDIKI_MUSTAFA_47')
